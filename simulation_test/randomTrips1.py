...
def get_options(args=None):
    ...
    if options.insertionDensity:
        ...
        options.insertionRate = [density * (length / 1000.0) for density in options.insertionDensity] #modify insertion rate here
...
def main(options):
        # Load Gaussian-based departure times
    gaussian_departures = []
    try:
        with open("gaussian_departures.txt", "r") as f:
            gaussian_departures = [int(line.strip()) for line in f]
    except FileNotFoundError:
        print("Gaussian departures file not found. Using default departures.", file=sys.stderr)
            ...
            if options.flows == 0:
                for i in range(len(times)-1):
                    ...
                    if options.binomial is None:
                        ...
                        for depart_time in sorted(gaussian_departures):
                            try:
                                origin, destination, intermediate = generate_origin_destination(trip_generator, options)
                                idx = generate_one(idx, depart_time, options.end, options.period[0], origin, destination, intermediate)
                            except Exception as exc:
                                if options.verbose:
                                    print(exc, file=sys.stderr)
